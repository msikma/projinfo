...
const makeCommonStructure = (rawData, fromFiles) => {
  ...
  if (!rawData) {
    return { type: fromFiles ? fromFiles : null, configData: {}, otherData: {} }
  }
...
const findLangFiles = (pathStr, fileTypes) => {
  const langFiles = Object.keys(fileTypes).map(lang => {
    const exts = fileTypes[lang].join('|')
    const pattern = fileTypes[lang].length > 1 ? `{*.+(${exts}),*/*.+(${exts})}` : `{*.${exts},*/*.${exts}}`
    const files = glob.sync(pattern, { cwd: pathStr, silent: true })
...
/** Returns Zeit project info from now.json. */
const getZeitData = (path) => {
  const nowFile = `${path}/now.json`
  const isNow = existsSync(nowFile)
  if (!isNow) return {}
  const nowData = require(nowFile)
  // Alias is a string or an array.
  const alias = nowData && nowData.alias
  const aliasArr = Array.isArray(alias) ? alias : [alias]
  // Add the .now.sh suffix to aliases without tld.
  const nowAlias = aliasArr.map(a => a.indexOf('.') === -1 ? `${a}.now.sh` : a).join(', ')
  return {
    isNow,
    nowData,
    nowAlias
...
/** Returns monorepo data for Lerna, Yarn or npm workspaces. */
const getMonorepoData = (path, pkgData) => {
  ...
  return {
    ...
    monorepoType: lernaExists ? 'lerna' : pkgData.packageManager === 'npm' ? 'npm' : 'yarn',

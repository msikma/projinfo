const fs = require('fs')
const { asyncFilter } = require('./async')
const { execCmd } = require('./exec')

// Bin file types for each language. (That I care about, anyway.)
const BIN_TYPES = {
  python: 'py',
  php: 'php',
  node: 'js'
...
const getBins = async (path, pkgData) => {
  ...
  const binFiles = await filterExecutable(filterFiles(fs.readdirSync(path), [BIN_TYPES[type]]))

  return { bins: [...pkgFiles, ...removeItem(binFiles, 'setup.py')] }
}

/** Checks if a file is executable. */
const checkExecutable = async file => {
  try {
    return await fs.promises.access(file, fs.constants.X_OK) === undefined
  }
  catch (err) {
    if (err.code === 'EACCES') {
      return false
...
/** Filters a list of files to only ones with the executable bit set. */
const filterExecutable = async (files) => {
  return asyncFilter(files, file => checkExecutable(file))
}
...
const getCommit = (quickCall = false) => {
  ...
  // List all branches to determine if this is the initial commit in the initial branch.
  const gitBranches = execCmd('git branch --format=\'%(refname:short)\'; exit 0').trim().split('\n')
  // This is the initial commit if there's only one branch and the count is '1'.
  const commitIsInitial = gitBranches.length === 1 && commitCommits === '1'
